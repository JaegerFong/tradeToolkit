...
try:
    ...
    PG_STORAGE_AVAILABLE = True
except ImportError as e:
    logger.error(f"❌ [ConfigManager] 导入 PgStorage 失败 (ImportError): {e}")
    import traceback
    logger.error(f"   堆栈: {traceback.format_exc()}")
    PG_STORAGE_AVAILABLE = False
    PgStorage = None
except Exception as e:
    logger.error(f"❌ [ConfigManager] 导入 PgStorage 失败 (Exception): {e}")
    import traceback
    logger.error(f"   堆栈: {traceback.format_exc()}")
    PG_STORAGE_AVAILABLE = False
    PgStorage = None
...
class ConfigManager:
    ...
    def _init_pg_storage(self):
        """初始化 PG 存储"""
        logger.info("🔧 [ConfigManager] 开始初始化 PG 存储...")

        if not PG_STORAGE_AVAILABLE:
            logger.warning("⚠️ [ConfigManager] PgStorage 不可用，无法使用 PG 存储")
            return

        # 检查是否启用 PG 存储
        use_pg_env = os.getenv("USE_PG_STORAGE", "false")
        use_pg = use_pg_env.lower() == "true"

        logger.info(f"🔍 [ConfigManager] USE_PG_STORAGE={use_pg_env} (解析为: {use_pg})")

        if not use_pg:
            logger.info("ℹ️ [ConfigManager] PG 存储未启用，将使用 JSON 文件存储")
            return

        try:
            pg_uri = os.getenv("TRADINGAGENTS_PG_URI", "").strip()
            database_name = os.getenv("TRADINGAGENTS_PG_DATABASE", "").strip() or "tradingagents"

            logger.info(f"🔍 [ConfigManager] TRADINGAGENTS_PG_URI={'已设置' if pg_uri else '使用默认连接'}")
            logger.info(f"🔍 [ConfigManager] TRADINGAGENTS_PG_DATABASE={database_name}")

            logger.info(f"🔄 [ConfigManager] 正在创建 PgStorage 实例...")
            self.pg_storage = PgStorage(
                connection_uri=pg_uri if pg_uri else None,
                database_name=database_name
            )

            if self.pg_storage.is_connected():
                logger.info(f"✅ [ConfigManager] PG 存储已启用: {database_name}.token_usage")
            else:
                self.pg_storage = None
                logger.warning("⚠️ [ConfigManager] PG 连接失败，将使用JSON文件存储")

        except Exception as e:
            logger.error(f"❌ [ConfigManager] PG 存储初始化失败: {e}", exc_info=True)
            self.pg_storage = None
    ...
    def add_usage_record(self, provider: str, model_name: str, input_tokens: int,
        ...
        # 优先使用PG存储
        if self.pg_storage and self.pg_storage.is_connected():
            logger.info(f"📊 [Token记录] 使用 PG 存储 (数据库: {self.pg_storage.database_name}, 表: {self.pg_storage.collection_name})")
            success = self.pg_storage.save_usage_record(record)
            if success:
                logger.info(f"✅ [Token记录] PG 保存成功: {provider}/{model_name}")
                return record
            else:
                logger.error(f"⚠️ [Token记录] PG 保存失败，回退到JSON文件存储")
        else:
            if self.pg_storage is None:
                logger.warning(f"⚠️ [Token记录] PG 存储未初始化 (pg_storage=None)")
                logger.warning(f"   💡 请检查环境变量: USE_PG_STORAGE={os.getenv('USE_PG_STORAGE', '未设置')}")
            elif not self.pg_storage.is_connected():
                logger.warning(f"⚠️ [Token记录] PG 未连接 (is_connected=False)")
    ...
    def get_usage_statistics(self, days: int = 30) -> Dict[str, Any]:
        ...
        if self.pg_storage and self.pg_storage.is_connected():
            try:
                stats = self.pg_storage.get_usage_statistics(days)
                provider_stats = self.pg_storage.get_provider_statistics(days)
                ...
                logger.error(f"⚠️ PG 统计获取失败，回退到JSON文件: {e}")
