...
class FinancialDataSyncService:
    ...
    async def _sync_source_financial_data(
        ...
        if not provider.is_available():
            logger.warning(f"{data_source} 数据源不可用")
        ...
        for i in range(0, len(symbols), batch_size):
            ...
            for symbol in batch_symbols:
                task = self._sync_symbol_financial_data(
                    ...
                    provider=provider, report_types=report_types
                )
                tasks.append(task)
            ...
            for j, result in enumerate(batch_results):
                ...
                if isinstance(result, Exception):
                    ...
                    stats.errors.append({
                        "symbol": symbol, "data_source": data_source,
                        "error": str(result), "timestamp": datetime.now(timezone.utc).isoformat()
                    })
                    ...
                    logger.debug(f"{symbol} 财务数据跳过 ({data_source})")
    ...
    async def _sync_symbol_financial_data(
        ...
        try:
            ...
            if not financial_data:
                logger.debug(f"{symbol} 无财务数据 ({data_source})")
                return False
    ...
    async def sync_single_stock(
        self, symbol: str, data_sources: List[str] = None
    ) -> Dict[str, bool]:
        """同步单只股票的财务数据"""
        if self.financial_service is None:
            await self.initialize()
        ...
        for data_source in data_sources:
            ...
            try:
                ...
                result = await self._sync_symbol_financial_data(
                    symbol=symbol, data_source=data_source,
                    provider=provider, report_types=["quarterly"]
                )
