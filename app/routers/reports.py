...
from ..core.pg_adapter import get_pg_db
from ..core.pg_models import StockBasicInfo
    ...
    try:
        ...
        async with async_session_factory() as session:
            ...
            for data_source in enabled_sources:
                result = await session.execute(
                    select(StockBasicInfo).where(
                        or_(
                            StockBasicInfo.symbol == code6,
                            StockBasicInfo.code == code6
                        ),
                        StockBasicInfo.source == data_source
                    )
                )
            ...
            if not stock_info:
                ...
                stock_info = result.scalar_one_or_none()
                if stock_info:
                    logger.warning(f"使用旧数据获取股票名称 {code6}")
...
# NOTE: analysis_reports 暂无 PG 模型，保留 MongoDB 查询
...
async def get_reports_list(
    ...
    try:
        ...
        # NOTE: analysis_reports 暂无 PG 模型，保留 MongoDB
        db = get_pg_db()
        ...
        async for doc in cursor:
            stock_code_val = doc.get("stock_symbol", "")
            stock_name = doc.get("stock_name")
            if not stock_name:
                stock_name = await get_stock_name(stock_code_val)

            market_type = doc.get("market_type")
            if not market_type:
                from tradingagents.utils.stock_utils import StockUtils
                market_info = StockUtils.get_market_info(stock_code_val)
                market_type_map = {
                    "china_a": "A股",
                    "hong_kong": "港股",
                    "us": "美股",
            ...
            report = {
                ...
                "title": f"{stock_name}({stock_code_val}) 分析报告",
...
async def get_report_detail(
    ...
    """获取报告详情 (analysis_reports + analysis_tasks 暂无 PG 模型)"""
    try:
        ...
        if not doc:
            logger.info(f"未在analysis_reports找到，尝试从analysis_tasks还原: {report_id}")
    ...
    try:
        ...
        if format == "json":
            ...
            if not report_exporter.pandoc_available:
                raise HTTPException(status_code=400, detail="Word 导出功能不可用。请安装 pandoc")
            ...
            if not report_exporter.pandoc_available:
                raise HTTPException(status_code=400, detail="PDF 导出功能不可用。请安装 pandoc")
