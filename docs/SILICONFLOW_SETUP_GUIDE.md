...
硅基流动（SiliconFlow）是一个高性价比的 AI 推理服务平台，提供多种开源大模型的 API 访问。本指南将帮助您在 tradeToolkit 中配置和使用硅基流动。
...
**示例**：
```bash
SILICONFLOW_API_KEY=replace_with_your_siliconflow_api_key
```
...
恭喜！您已经成功配置了硅基流动。现在可以在 tradeToolkit 中使用硅基流动的模型进行 AI 分析了。
