...
chcp 65001 >nul
title tradeToolkit 启动器

echo ========================================
echo   tradeToolkit 绿色版启动器
echo ========================================
echo.
...
chcp 65001 >nul
title tradeToolkit 停止器

echo ========================================
echo   tradeToolkit 绿色版停止器
echo ========================================
echo.
...
; tradeToolkit Windows 安装器脚本

!include "MUI2.nsh"

; 基本信息
Name "tradeToolkit"
OutFile "tradeToolkit-Setup.exe"
InstallDir "$PROGRAMFILES64\tradeToolkit"
...
Section "主程序" SecMain
    ...
    ; 创建快捷方式
    CreateDirectory "$SMPROGRAMS\tradeToolkit"
    CreateShortcut "$SMPROGRAMS\tradeToolkit\tradeToolkit.lnk" "$INSTDIR\start.bat"
    CreateShortcut "$DESKTOP\tradeToolkit.lnk" "$INSTDIR\start.bat"
    
    ; 写入卸载信息
    WriteUninstaller "$INSTDIR\Uninstall.exe"
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\tradeToolkit" \
                     "DisplayName" "tradeToolkit"
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\tradeToolkit" \
...
Section "Uninstall"
    ...
    ; 删除快捷方式
    Delete "$DESKTOP\tradeToolkit.lnk"
    RMDir /r "$SMPROGRAMS\tradeToolkit"
    
    ; 删除注册表
    DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\tradeToolkit"
SectionEnd
```
...
def build_installer():
    ...
    if result.returncode == 0:
        print("✅ 安装器构建成功！")
        print(f"📦 输出文件: tradeToolkit-Setup.exe")
    else:
        print("❌ 安装器构建失败")
        print(result.stderr)
        sys.exit(1)
