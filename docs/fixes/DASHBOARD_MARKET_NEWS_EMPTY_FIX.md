...
**添加定时任务**（每小时同步一次）：
```cron
0 * * * * cd /path/to/tradeToolkit && .venv/bin/python scripts/sync_market_news.py >> logs/news_sync.log 2>&1
```
...
3. 触发器：每小时
4. 操作：启动程序
   - 程序：`d:\code\tradeToolkit\.venv\Scripts\python.exe`
   - 参数：`scripts/sync_market_news.py`
   - 起始于：`d:\code\tradeToolkit`
