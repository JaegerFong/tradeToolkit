...
tradeToolkit采用本地构建Docker镜像的方式，而不是提供预构建镜像。本文档详细说明了Docker镜像的构建过程、优化方法和常见问题解决方案。
...
# 分析构建层
docker history tradeToolkit:latest

# 查看镜像大小
docker images tradeToolkit
```
...
```bash
# 为镜像打标签
docker build -t tradeToolkit:v0.1.7 .
docker build -t tradeToolkit:latest .

# 推送到私有仓库 (可选)
docker tag tradeToolkit:latest your-registry/tradeToolkit:latest
docker push your-registry/tradeToolkit:latest
```
...
# 扫描安全漏洞
docker scan tradeToolkit:latest
```
