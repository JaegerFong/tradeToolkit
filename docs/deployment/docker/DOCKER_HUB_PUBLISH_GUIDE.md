...
本文档说明如何构建 tradeToolkit 的 Docker 镜像并发布到 Docker Hub。
...
# 完整示例
./scripts/publish-docker-images.sh  v1.0.0-preview
```
...
```powershell
# Windows
.\scripts\publish-docker-images.ps1 -DockerHubUsername ""
...
services:
  tradingagents-backend:
    image: /tradingagents-backend:latest  # 替换为你的用户名
    
  tradingagents-frontend:
    image: /tradingagents-frontend:latest  # 替换为你的用户名
```
