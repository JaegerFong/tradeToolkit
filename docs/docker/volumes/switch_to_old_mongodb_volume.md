...
| 项目 | 数据卷名称 | 状态 | 数据 |
|------|-----------|------|------|
| **当前运行的容器** | `tradeToolkit_tradingagents_mongodb_data_v1` | ✅ 正在使用 | ❌ 空的（只有3个LLM配置） |
...
#### 新数据卷 `tradeToolkit_tradingagents_mongodb_data_v1`（空的）
...
- ✅ 新数据卷 `tradeToolkit_tradingagents_mongodb_data_v1` 是空的（只有3个LLM）
