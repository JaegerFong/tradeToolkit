...
欢迎来到本项目的文档中心。本文档覆盖配置管理、模型目录、大模型配置、股票详情、单股同步、日志排查、部署升级等核心内容。
...
- **反馈方式**：在你自己的仓库/项目管理工具中提交 Issue（建议附日志与复现步骤）
- **上游来源**：本项目基于开源项目 `TauricResearch/TradingAgents` 进行二次开发（详见仓库内许可证与归属说明）
