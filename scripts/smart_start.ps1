# tradeToolkit 智能Docker启动脚本 (Windows PowerShell版本)
# 功能：自动判断是否需要重新构建Docker镜像
# 使用：powershell -ExecutionPolicy Bypass -File scripts\smart_start.ps1
# 
# 判断逻辑：
# 1. 检查是否存在tradeToolkit镜像
# 2. 如果镜像不存在 -> 执行构建启动
# 3. 如果镜像存在但代码有变化 -> 执行构建启动  
# 4. 如果镜像存在且代码无变化 -> 快速启动

Write-Host "=== tradeToolkit Docker 智能启动脚本 ===" -ForegroundColor Green
Write-Host "适用环境: Windows PowerShell" -ForegroundColor Cyan

# 检查是否有镜像
$imageExists = docker images | Select-String "tradeToolkit"
